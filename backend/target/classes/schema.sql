-- ============================================
-- 本地开发 & Test Profile 自动建表脚本
-- 使用 CREATE TABLE IF NOT EXISTS，不会破坏已有数据
-- ============================================
...
-- 4. 同步任务文件明细表
CREATE TABLE IF NOT EXISTS `sync_task_file` (
    `id`          BIGINT       AUTO_INCREMENT PRIMARY KEY COMMENT '主键',
...
-- 初始化默认管理员账号（username 唯一索引保证不会重复插入）
INSERT IGNORE INTO `sys_user` (`username`, `password`, `nickname`, `status`, `role`)
